...
function returnTen(str){
  // Solution code here...
  let lastTenStr = str.slice(-10, 11);
  const result = [];
  for(let i = 0; i<lastTenStr.length; i++){
    result[i]=lastTenStr.slice(i, i+1);
  }
  return result;
}
...
const containsAnd = (arr) => {
  // Solution code here...
  const re = /and/g;
  const result =arr.filter(word => word.match(re));
  return result;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const result = arr.filter(item => !(forbiddenValues.includes(item)));
  return result;
};
