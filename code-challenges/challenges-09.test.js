...
const addValues = (arr) => {
  // Solution code here...
  let initValue = 0;
  let result = arr.reduce((runningTotal, currentValue) => {
...
const addPurchases = (arr) => {
  ...
  let result = Object.values(arr).reduce((runningTotal, {purchasePrice}) => {
    return runningTotal + purchasePrice;
  }, initValue);
  return result;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let initValue = 0;
  let result = arr.reduce((runningTotal) => {
...
const returnNames = (arr) => {
  ...
  let initValue = '';
  let names = [];
  Object.values(arr).reduce((runningTotal, {name}) => {
    names.push(`${name}`);
  }, initValue);
  return names;
};
...
const reversedString = (str) => {
  ...
  for(let i = 1; i <= str.length; i++){
    strArr.push(str.slice(-i,str.length-i+1));
  }

  let result = '';

  strArr.reduce((runningTotal, currentValue) => {
