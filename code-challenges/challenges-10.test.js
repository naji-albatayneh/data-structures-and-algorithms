...
const getNames = (arr) => {
  ...
  let initValue = '';
  const reversedNames = [];
  Object.values(arr).map(({name}) => {
    const reversedStr = [];
    let result = '';

    for(let i = 1; i <= name.length; i++){
      reversedStr.push(name.slice(-i,name.length-i+1));
    }

    reversedStr.reduce((runningTotal, currentValue) => {
      result = result + currentValue;
    }, initValue);

    reversedNames.push(result);
  });
...
let findShortest = (data) => {
  ...
  let shortestHeight = parseInt(data[0].height);
  Object.values(data).map(({height, name}) => {
    if(parseInt(height)<shortestHeight){
      shortestHeight = parseInt(height);
      shortestName= name;
    }
  });
  return shortestName;
